/* lazy.c generated by valac 0.52.0, the Vala compiler
                           ...
                           GError** error)
{
	...
	if (_tmp1_ != NULL) {
		...
		if (_tmp2_ == GEE_LAZY_FUTURE_STATE_EVAL) {
			while (TRUE) {
				GeeLazyFutureState _tmp3_;
				_tmp3_ = self->priv->_state;
				if (!(_tmp3_ == GEE_LAZY_FUTURE_STATE_EVAL)) {
					break;
				}
...
gee_lazy_future_real_wait_async_co (GeeLazyFutureWaitAsyncData* _data_)
{
	...
	if (_data_->_tmp1_ != NULL) {
		...
		if (_data_->_tmp2_ == GEE_LAZY_FUTURE_STATE_EVAL) {
			...
			gee_future_source_func_array_element_init (&_data_->_tmp3_, _gee_lazy_future_real_wait_async_co_gsource_func, _data_, NULL);
			_vala_array_add1 (&_data_->self->priv->_when_done, &_data_->self->priv->_when_done_length1, &_data_->self->priv->__when_done_size_, &_data_->_tmp3_);
			_data_->_state_ = 1;
	...
	_data_->_tmp4_ = _data_->self->priv->_lazy;
	_data_->_tmp5_ = gee_lazy_get_value (_data_->_tmp4_);
                            ...
                            gpointer klass_data)
{
	...
	g_object_class_install_property (G_OBJECT_CLASS (klass), GEE_LAZY_FUTURE_EXCEPTION_PROPERTY, gee_lazy_future_properties[GEE_LAZY_FUTURE_EXCEPTION_PROPERTY] = g_param_spec_pointer ("exception", "exception", "exception", G_PARAM_STATIC_STRINGS | G_PARAM_READABLE));
}
