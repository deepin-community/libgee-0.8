/* promise.c generated by valac 0.52.0, the Vala compiler
                              ...
                              GError** error)
{
	...
	switch (_tmp3_) {
		...
		{
			GError* _tmp5_;
			_tmp5_ = g_error_new_literal (GEE_FUTURE_ERROR, GEE_FUTURE_ERROR_EXCEPTION, "Exception has been thrown");
...
gee_promise_future_real_wait_async_co (GeePromiseFutureWaitAsyncData* _data_)
{
	...
	if (_data_->_tmp1_ == GEE_PROMISE_FUTURE_STATE_INIT) {
		...
		gee_future_source_func_array_element_init (&_data_->_tmp2_, _gee_promise_future_real_wait_async_co_gsource_func, _data_, NULL);
		_vala_array_add2 (&_data_->self->priv->_when_done, &_data_->self->priv->_when_done_length1, &_data_->self->priv->__when_done_size_, &_data_->_tmp2_);
		_data_->_state_ = 1;
		...
		_data_->state = _data_->_tmp3_;
	} else {
		g_mutex_unlock (&_data_->self->priv->_mutex);
	}
	_data_->_tmp4_ = _data_->state;
	_vala_assert (_data_->_tmp4_ != GEE_PROMISE_FUTURE_STATE_INIT, "state != State.INIT");
	_data_->_tmp5_ = _data_->state;
	switch (_data_->_tmp5_) {
		case GEE_PROMISE_FUTURE_STATE_ABANDON:
		{
			_data_->_tmp6_ = g_error_new_literal (GEE_FUTURE_ERROR, GEE_FUTURE_ERROR_ABANDON_PROMISE, "Promise has been abandon");
		...
		case GEE_PROMISE_FUTURE_STATE_EXCEPTION:
		{
			_data_->_tmp7_ = g_error_new_literal (GEE_FUTURE_ERROR, GEE_FUTURE_ERROR_EXCEPTION, "Exception has been thrown");
		...
		case GEE_PROMISE_FUTURE_STATE_READY:
		{
			_data_->_tmp8_ = _data_->self->priv->_value;
