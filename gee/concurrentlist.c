/* concurrentlist.c generated by valac 0.52.0, the Vala compiler
                                  ...
                                  gboolean force)
{
	...
	while (TRUE) {
		...
		curr_next = _tmp4_;
		_tmp6_ = next_state;
		if (_tmp6_ != GEE_CONCURRENT_LIST_STATE_MARKED) {
			_tmp5_ = TRUE;
		} else {
			gboolean _tmp7_ = FALSE;
			GeeConcurrentListState _tmp8_;
			_tmp8_ = curr_state;
			if (_tmp8_ == GEE_CONCURRENT_LIST_STATE_MARKED) {
				GeeConcurrentListNode* _tmp9_;
				GeeConcurrentListNode* _tmp10_;
				_tmp9_ = curr_next;
				_tmp10_ = next;
		...
		if (_tmp11_ == _tmp12_) {
			...
			_tmp13_ = next;
			gee_concurrent_list_node_help_marked (_tmp13_, *curr);
		}
		_tmp14_ = curr_next;
		_tmp15_ = _gee_concurrent_list_node_ref0 (_tmp14_);
		_gee_concurrent_list_node_unref0 (next);
		next = _tmp15_;
		_gee_concurrent_list_node_unref0 (curr_next);
	}
	_tmp16_ = next;
	success = _tmp16_ != NULL;
	if (success) {
		_tmp17_ = TRUE;
	} else {
		_tmp17_ = force;
                                 ...
                                 GeeConcurrentListNode* next)
{
	...
	while (TRUE) {
		...
		if (_tmp2_ == GEE_CONCURRENT_LIST_STATE_FLAGGED) {
			GeeConcurrentListNode* _tmp3_;
			_tmp3_ = prev_next;
			gee_concurrent_list_node_help_flagged (_tmp3_, prev);
		} else {
			gboolean _result_ = FALSE;
			...
			prev_next = _tmp5_;
			_tmp6_ = prev_state;
			if (_tmp6_ == GEE_CONCURRENT_LIST_STATE_FLAGGED) {
				GeeConcurrentListNode* _tmp7_;
				_tmp7_ = prev_next;
				gee_concurrent_list_node_help_flagged (_tmp7_, prev);
			}
		...
		while (TRUE) {
			...
			if (!_result_) {
				...
				if (_tmp6_ == GEE_CONCURRENT_LIST_STATE_FLAGGED) {
					GeeConcurrentListNode* _tmp7_;
					_tmp7_ = next_node;
					gee_concurrent_list_node_help_flagged (self, _tmp7_);
				}
			}
			_gee_concurrent_list_node_unref0 (next_node);
		}
                                   ...
                                   GeeConcurrentListNode* * prev_node)
{
	...
	while (TRUE) {
		...
		if (_tmp3_ == self) {
			...
			_tmp4_ = result_state;
			_tmp2_ = _tmp4_ == GEE_CONCURRENT_LIST_STATE_FLAGGED;
		} else {
			_tmp2_ = FALSE;
		}
		if (_tmp2_) {
		...
		_tmp5_ = gee_concurrent_list_node_search_for (self->priv->g_type, (GBoxedCopyFunc) self->priv->g_dup_func, (GDestroyNotify) self->priv->g_destroy_func, self, prev_node);
		if (!_tmp5_) {
