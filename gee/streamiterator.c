/* streamiterator.c generated by valac 0.52.0, the Vala compiler
                               ...
                               GDestroyNotify func_target_destroy_notify)
{
	...
	switch (_tmp5_) {
		...
		case GEE_TRAVERSABLE_STREAM_YIELD:
		{
			...
			gboolean _tmp7_;
			gboolean _tmp8_;
			_tmp6_ = self->priv->_outer;
			_tmp7_ = gee_iterator_get_valid (_tmp6_);
			_tmp8_ = _tmp7_;
			self->priv->_need_next = !_tmp8_;
			break;
		}
		case GEE_TRAVERSABLE_STREAM_CONTINUE:
		{
			...
			gboolean _tmp10_;
			gboolean _tmp11_;
			_tmp9_ = self->priv->_outer;
			_tmp10_ = gee_iterator_get_valid (_tmp9_);
			_tmp11_ = _tmp10_;
			if (_tmp11_) {
				GeeStreamFunc _tmp12_;
				gpointer _tmp12__target;
				GeeTraversableStream _tmp13_;
				GeeLazy* _tmp14_;
				GeeLazy* _tmp15_ = NULL;
				GeeTraversableStream _tmp16_;
				GeeTraversableStream _tmp17_;
				_tmp12_ = self->priv->_func;
				_tmp12__target = self->priv->_func_target;
				_tmp13_ = self->priv->_state;
				_tmp14_ = gee_lazy_new (g_type, (GBoxedCopyFunc) g_dup_func, (GDestroyNotify) g_destroy_func, _____lambda8__gee_lazy_func, g_object_ref (self), g_object_unref);
				_tmp16_ = _tmp12_ (_tmp13_, _tmp14_, &_tmp15_, _tmp12__target);
				_gee_lazy_unref0 (self->priv->_current);
				self->priv->_current = _tmp15_;
				self->priv->_state = _tmp16_;
                                  ...
                                  gpointer f_target)
{
	...
	_result_ = TRUE;
	_tmp14_ = self->priv->_outer_value;
	_tmp15_ = _gee_lazy_ref0 (_tmp14_);
	outer_value = _tmp15_;
	while (TRUE) {
		GeeIterator* _tmp16_;
		GeeStreamFunc _tmp17_;
		...
		_tmp16_ = outer;
		_tmp17_ = func;
		_tmp17__target = func_target;
		_tmp18_ = gee_stream_iterator_yield_next (self->priv->a_type, (GBoxedCopyFunc) self->priv->a_dup_func, (GDestroyNotify) self->priv->a_destroy_func, self->priv->g_type, (GBoxedCopyFunc) self->priv->g_dup_func, (GDestroyNotify) self->priv->g_destroy_func, _tmp16_, _tmp17_, _tmp17__target, &state, &need_next, &outer_value);
		_gee_lazy_unref0 (next_current);
		next_current = _tmp18_;
		_tmp19_ = next_current;
		if (!(_tmp19_ != NULL)) {
			break;
		}
		_tmp20_ = next_current;
		next_current = NULL;
		_gee_lazy_unref0 (current);
		current = _tmp20_;
		_tmp21_ = current;
		...
		_tmp23_ = _tmp22_;
		_tmp24_ = ((_tmp23_ != NULL) && (self->priv->a_dup_func != NULL)) ? self->priv->a_dup_func ((gpointer) _tmp23_) : ((gpointer) _tmp23_);
		if (!f (_tmp24_, f_target)) {
			_result_ = FALSE;
			break;
		}
	}
	_tmp25_ = state;
	self->priv->_state = _tmp25_;
	self->priv->_need_next = need_next;
	self->priv->_finished = _result_;
	_tmp26_ = current;
	current = NULL;
	_gee_lazy_unref0 (self->priv->_current);
	self->priv->_current = _tmp26_;
	_tmp27_ = outer_value;
	outer_value = NULL;
	_gee_lazy_unref0 (self->priv->_outer_value);
	self->priv->_outer_value = _tmp27_;
	result = _result_;
